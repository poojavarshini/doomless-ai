...
function Options() {
  ...
  return <main className="panel options"><div className="brand-lockup"><img src="/icons/icon-48.png" alt="" /><div><span className="eyebrow">DoomLess AI</span><h1>Preferences & privacy</h1></div></div><p className="lede">Personal relevance changes with you. Intrinsic quality and behavioral-risk scores do not.</p>
    <form onSubmit={submit}>
      <section><h2>Analysis controls</h2><label className="check"><input type="checkbox" checked={settings.enabled} onChange={(event) => update("enabled", event.target.checked)} /> Enable Reel analysis</label><label className="check"><input type="checkbox" checked={settings.analyzeOnHover} onChange={(event) => update("analyzeOnHover", event.target.checked)} /> Analyze after an 800ms hover</label><label className="check"><input type="checkbox" checked={settings.analyzeVisible} onChange={(event) => update("analyzeVisible", event.target.checked)} /> Analyze the active visible Reel</label><label className="check"><input type="checkbox" checked={settings.demoMode} onChange={(event) => update("demoMode", event.target.checked)} /> Demo mode — show pre-analyzed beta examples on detected Reels</label>{settings.demoMode && <p className="demo-note">Demo mode is simulated and clearly labeled. It does not describe the real Reel underneath the overlay.</p>}<label>Backend URL<input value={settings.apiBaseUrl} onChange={(event) => update("apiBaseUrl", event.target.value)} /></label></section>
      <section><h2>Your interests</h2><div className="chips">{choices.map((choice) => <button className={settings.preferences.interests.includes(choice) ? "chip selected" : "chip"} type="button" aria-pressed={settings.preferences.interests.includes(choice)} onClick={() => toggleInterest(choice)} key={choice}>{choice}</button>)}</div><label>Professional field<input value={settings.preferences.professionalField} onChange={(event) => update("preferences", { ...settings.preferences, professionalField: event.target.value })} /></label><label>Learning goals (comma-separated)<input value={settings.preferences.learningGoals.join(", ")} onChange={(event) => update("preferences", { ...settings.preferences, learningGoals: splitList(event.target.value) })} /></label><label>Topics to avoid (comma-separated)<input value={settings.preferences.topicsToAvoid.join(", ")} onChange={(event) => update("preferences", { ...settings.preferences, topicsToAvoid: splitList(event.target.value) })} /></label><label>Maximum daily Reel time<input type="number" min="1" max="240" value={settings.preferences.maximumDailyReelMinutes} onChange={(event) => update("preferences", { ...settings.preferences, maximumDailyReelMinutes: Number(event.target.value) })} /></label><label>Education balance: {settings.preferences.educationBalance}%<input type="range" min="0" max="100" value={settings.preferences.educationBalance} onChange={(event) => update("preferences", { ...settings.preferences, educationBalance: Number(event.target.value) })} /></label></section>
